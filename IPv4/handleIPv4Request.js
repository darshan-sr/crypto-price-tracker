...
async function handleIPv4Request(req, res) {
  ...
  try {
    ...
    // HTML route
    if (method === "GET" && url === "/") {
      fs.readFile("IPv4/index.html", (err, data) => {
        if (err) {
          res.statusCode = 500;
    ...
    if (method === "GET" && url.startsWith("/coin")) {
      fs.readFile("IPv4/coin.html", (err, data) => {
        if (err) {
          res.statusCode = 500;
          res.setHeader("Content-Type", "text/plain");
          res.end("Internal Server Error");
...
exports.handleIPv4Request = handleIPv4Request;
